(() => {
  ...
  function infoPage() {
    ...
    return appShell(`
      ...
      <section class="section">
        <article class="card rental-card"><div><div class="card-top"><div><div class="mini-label">תחבורה · ${esc(rental.provider)}</div><h3>${esc(rental.title)}</h3></div><span class="status pending">${esc(rental.status)}</span></div><div class="grid grid-2" style="margin-top:15px"><div class="soft-box"><div class="mini-label">הגעה</div><b>${esc(rental.pickup)}</b></div><div class="soft-box"><div class="mini-label">חזרה</div><b>${esc(rental.return)}</b></div></div><div class="info-row" style="margin-top:14px"><span><b>${esc(rental.vehicle)}</b></span><span>${esc(rental.deposit)}</span></div><p>${esc(rental.requirements)}</p><div class="warning-box">${esc(rental.warning)}</div></div>${rental.document ? `<a class="btn btn-dark btn-small" href="${esc(rental.document)}" target="_blank" rel="noopener">פתיחת המסמך</a>` : `<span class="status pending">פרטי התחבורה טרם צורפו</span>`}</article>
      </section>
